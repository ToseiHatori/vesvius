#!/usr/bin/env python3
"""
nnUNet Preprocessing for Vesuvius Surface Detection (Local Version)

Based on: surface-nnunet-preprocessing.ipynb
Purpose: Preprocess data for 3d_fullres configuration (Host Baseline)

Usage:
    python surface-nnunet-preprocessing-local.py

Requirements:
    pip install nnunetv2 nibabel tifffile tqdm scikit-image
"""
...
PROJECT_DIR = Path("/home/ben/Dev/vesvius")

# Competition data path (downloaded via Kaggle API)
INPUT_DIR = PROJECT_DIR / "data" / "vesuvius-challenge-surface-detection"

# Pre-prepared nnUNet preprocessed dataset (set to non-existent path to force fresh preprocessing)
PREPARED_PREPROCESSED_PATH = Path("/nonexistent")

# Working directories (local paths with sufficient disk space)
WORKING_DIR = PROJECT_DIR / "nnunet_work"
...
def main():
    print("=" * 60)
    print("VESUVIUS PREPROCESSING - 3d_fullres (LOCAL)")
    print("=" * 60)
    ...
    if not INPUT_DIR.exists():
        ...
        print("  cd /home/ben/Dev/vesvius/data")
        print("  kaggle competitions download -c vesuvius-challenge-surface-detection")
        print("  unzip vesuvius-challenge-surface-detection.zip")
        return False
    ...
    if free_gb < 300:
        print(f"WARNING: Less than 300GB free. Preprocessing may fail.")
